---
title: "Calculating the wildfire hazard potential score for every school district"
format: html
...
To describe wildfire conditions for school districts, we use the Wildfire Hazard Potential for the United States (270-m), version 2023, 4th edition. This dataset is a raster covering the continental United States, describing wildfire hazard potential in categories from 1-7. The data was published in 2023 and is based off of 2020 landscape conditions. The intention of using this dataset is to find the mean wildfire hazard potential score for each school district in California. To provide a basis for comparison, we'll repeat the process with the version of the dataset published in 2015, which is based off of 2012 landscape conditions.
...
# Working with 2023 data

The raster is categorical on a scale from 1-7, but the values are read in as integers, so we'll convert it to a factor for reprojecting and cropping the raster. This prevents fractional values from occurring. Our second step is to reproject the whp raster to be on the same coordinate reference system as our school districts.
...
## Calculate the value of wildfire hazard potential for every school district
...
To provide a comparison for the 2023 data, we use the 2015 version of the dataset, which is based off of 2012 landscape conditions. We are going to repeat the same process with this version of the data to assign historic wildfire hazard potential scores to school districts.

Limitation- These two points in time aren't that far from each other. Even though we expect wildfires to grow larger and more frequent, this data might not show it. The average wildfire hazard potential score is actually higher for the 2015 data.
...
## Calculate the value of wildfire hazard potential for every school district based on 2015 data
