...
for (code in districts$CDSCode) {
  ...
  tryCatch({
    ...
    districts_temp <- request %>% 
      ...
      group_by(CDSCode, dt) %>% 
      summarize(threshold = ifelse(any(val > 308.87), 1, 0)) %>%
...
# write to data folder for tomorrow
write_csv(error_districts, here("data", "extreme_heat", "intermediate", "error_districts.csv"))
