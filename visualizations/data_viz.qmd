...
ggplot(data = districts_sub %>% 
            mutate(slr_score_hist = as.factor(slr_score_hist))) +
  # plot sea level rise districts with a fill
  geom_sf(aes(fill = slr_score_hist),
          lwd = NA) +
  scale_fill_brewer(type = "seq", palette = "Purples", name = "",
                    guide = guide_legend(reverse = TRUE)) +
  # plot all districts with no fill and black boundaries on top
  geom_sf(data = ca, fill = NA, color = "black") +
  # crop map extent to the districts expected to experience sea level rise
  coord_sf(xlim = c(slr_bbox["xmin"], slr_bbox["xmax"]),
           ylim = c(slr_bbox["ymin"], slr_bbox["ymax"]),     
  ...
  theme(legend.justification = c(0, 1),
        ...
        plot.title.position = "plot",
        panel.border = element_rect(fill = NA)) +
  annotation_scale(style = "ticks", unit_category = "imperial", 
                   width_hint = .4, pad_x = unit(1.3, "in"), pad_y = unit(.1, "in")) +
  annotation_north_arrow(style = north_arrow_orienteering(),
                         height = unit(.4, "in"),
                         width = unit(.25, "in"),
                         pad_x = unit(1.3, "in"),
